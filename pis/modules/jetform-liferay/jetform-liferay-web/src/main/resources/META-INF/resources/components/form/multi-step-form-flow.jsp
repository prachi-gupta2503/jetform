...
<%
	...
	for(HtmlFormWrapper form : forms){
		//LOGGER.info("######################################## "+form.getFormClass());
		if(i==0){
%>
								<li class="active" id="<%=form.getFormClass().getSimpleName().toLowerCase()%>"><strong><%=form.getTitle()%></strong></li>
<%
		}else{
%>
								<li id="<%=form.getFormClass().getSimpleName().toLowerCase()%>"><strong><%=form.getTitle()%></strong></li>
<%		
		}
		i++;
	}
...
%>
							<fieldset id="<%=formId%>">
								<liferay-util:include page="/components/form/multi-step-form-add.jsp" servletContext="<%=application%>">
									...
									<liferay-util:param name="formClass" value="<%=form.getFormClass().getName()%>" />
