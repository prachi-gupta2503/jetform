...
import com.adjecti.dashboard.widget.service.persistence.impl.constants.OFBPersistenceConstants;
 ...
 * @generated
 */
@Component(service = DashboardWidgetPersistence.class)
public class DashboardWidgetPersistenceImpl
	...
	public DashboardWidget fetchByportletName(
		...
		if (useFinderCache) {
			result = finderCache.getResult(
				_finderPathFetchByportletName, finderArgs, this);
		}
	...
	protected void cacheUniqueFindersCache(
		...
		finderCache.putResult(
			_finderPathFetchByUUID_G, args, dashboardWidgetModelImpl, false);

		args = new Object[] {dashboardWidgetModelImpl.getPortletName()};

		finderCache.putResult(
			_finderPathCountByportletName, args, Long.valueOf(1), false);
		finderCache.putResult(
	...
	public DashboardWidget updateImpl(DashboardWidget dashboardWidget) {
		...
		if (isNew && (dashboardWidget.getCreateDate() == null)) {
			if (serviceContext == null) {
				dashboardWidget.setCreateDate(now);
			}
			else {
		...
		if (!dashboardWidgetModelImpl.hasSetModifiedDate()) {
			if (serviceContext == null) {
				dashboardWidget.setModifiedDate(now);
			}
			else {
	...
	public void activate(BundleContext bundleContext) {
		...
		_argumentsResolverServiceRegistration = _bundleContext.registerService(
			...
			new DashboardWidgetModelArgumentsResolver(),
			MapUtil.singletonDictionary(
				"model.class.name", DashboardWidget.class.getName()));
	...
	private static class DashboardWidgetModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(
					dashboardWidgetModelImpl, columnNames, original);
			}
		...
		private Object[] _getValue(
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] =
						dashboardWidgetModelImpl.getColumnOriginalValue(
							columnName);
				}
				else {
