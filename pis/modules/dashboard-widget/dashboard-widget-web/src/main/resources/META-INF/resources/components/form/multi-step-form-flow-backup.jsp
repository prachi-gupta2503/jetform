...
<%
	...
	String formFlowClassPK = ParamUtil.getString(request, "formClassPK", "0");
	...
	HtmlFormWrapper form=jetFormBuilderLocalService.getHtmlFormByClass(formClass);
...
groupIndex=1;
for(HtmlElementGroupWrapper group:form.getElementGroups()){
	//LOGGER.info("Group : "+group.getName()+"/"+group.getId());
	String liClass="";
	if(groupIndex==1){
...
<%		
		...
		for(HtmlElementWrapper element:group.getElements()){
			LOGGER.info("element.getControlType() : "+element.getControlType());
			if(element.isControlType(HtmlElement.ControlType.form)){
				...
				if(stepForm.isFormFlowProcess()){
					...
					if(formFlowClassPK!=null && !formFlowClassPK.equals("0")){
						Object entity=jetFormBuilderLocalService.readObject(stepForm.getEntityClass(), formFlowClassPK);
						stepForm.setEntity(entity);
						stepForm.setPrimaryKey(formFlowClassPK);
					}
				}
				if(nameSpace){
				...
				//LOGGER.info("formClass : "+stepForm.getFormClass().getSimpleName()+", formId: "+formId+", formPage: "+formPage);
				if(nameSpace){
%>
...
--%>
									...
									<liferay-util:include page="<%=formPage%>" servletContext="<%=application%>">
										...
										<liferay-util:param name="formFlowClassPK" value="<%=formFlowClassPK%>" />
...
<%				
				...
				//LOGGER.info("element.getControlType() 2 : "+element.getControlType());
				formIndex++;
				String formId="form".concat(String.valueOf(formIndex));
				HtmlFormWrapper stepForm=null;
				
				LOGGER.info("element.getFormClass() : "+element.getFormClass()+", element.getRelation().getModelClass() : "+element.getRelation().getModelClass());
				...
				//stepForm.setElementNamespace(stepForm.getId());
				request.setAttribute(formId, stepForm);
				LOGGER.info(HtmlElement.ControlType.list.name()+ ", stepForm : "+stepForm);
				//request.setAttribute("form", stepForm);
				request.setAttribute("formElement", element);
