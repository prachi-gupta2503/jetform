...
<%
	...
	if(childFormElement!=null){
		...
		try{
			...
			childForm = jetFormBuilderLocalService.getHtmlFormByClass(childFormElement.getFormClass(), childFormClassPK);
			//LOGGER.info("childForm : " + childForm);
		}catch(Exception e){}
		
		if(childForm==null){
			childForm=jetFormBuilderLocalService.getHtmlFormByClass(childFormElement.getFormClass());
	...
	if(childForm!=null){
		...
		entityClass=childForm.getEntityClass();
	}
...
function <portlet:namespace/><%=form.getId()%>editHandleOnClick(actionSource){
	...
	var url="<%=addRenderURL%>&<portlet:namespace/>dialogId="+dialogId+"&<portlet:namespace/>cancelFunc=<portlet:namespace /><%=form.getId()%>closeDialog";
