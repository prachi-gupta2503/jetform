(function($) {
    ...
    $.fn.orgChart.defaults = {
        data: [{organizationId:1, name:'Root', parentOrganizationId: 0}],
        showControls: false,
        allowEdit: false,
        onAddNode: null,
        onDeleteNode: null,
    ...
    function OrgChart($container, opts){
        ...
        var data = opts.data;
         subListData = JSON.stringify(opts.data);
        var nodes = {};
        var rootNodes = [];
        this.opts = opts;
        ...
        this.newNode = function(parentId){
            ...
            self.addNode({organizationId: nextId, name: '', parentOrganizationId: parentId});
        }

        this.addNode = function(data){
            var newNode = new Node(data);
            nodes[data.organizationId] = newNode;
            nodes[data.parentOrganizationId].addChild(newNode);

            self.draw();
            self.startEdit(data.organizationId);
        }

        this.deleteNode = function(id){
            for(var i=0;i<nodes[id].children.length;i++){
                self.deleteNode(nodes[id].children[i].data.organizationId);
            }
            nodes[nodes[id].data.parentOrganizationId].removeChild(id);
            delete nodes[id];
            self.draw();
        }
        ...
        for(var i in data){
            ...
            nodes[data[i].organizationId] = node;
        }

        // generate parent child tree
        for(var i in nodes){
            if(nodes[i].data.parentOrganizationId == 0){
                rootNodes.push(nodes[i]);
            }
            else{
                nodes[nodes[i].data.parentOrganizationId].addChild(nodes[i]);
            }
        }
    ...
    function Node(data){
        ...
        this.removeChild = function(id){
            for(var i=0;i<self.children.length;i++){
                if(self.children[i].data.organizationId == id){
        ...
        this.formatNode = function(opts){
        	...
        	//alert(subListData.split(data.organizationId).length-1)
            ...
            if(opts.showControls && (subListData.split(data.organizationId).length-1)>2){
            ...
            return "<div class='node' node-id='"+this.data.organizationId+"'>"+nameString+descString+buttonsHtml+"</div>";
        }
    }
