...
%>	
	<div class="card form-card" style="width:60%; margin-left:45px;">
		...
		<div class="card-body">
			<div id="status-report" style="display:none;overflow:auto;">
			</div>
			<div class="input-group">
				<label class="form-control">
					<span id="<portlet:namespace />Import_fileName">Upload a document</span>
					<a id="<portlet:namespace />Import_uploadBtn" style="float: right;"><img src="<%=request.getContextPath()%>/images/upload.png" style="width:80%;"></a>
					<a id="<portlet:namespace />Import_clearBtn" style="float: right; display:none;color:red;"><img src="<%=request.getContextPath()%>/images/close.png" style="width:80%;"></a>
				</label>
...
%>
	<div class="container" style="margin-top:50px">
		<div class="row">
			<div class="col-md-12" id="status-report" style="display:none;overflow:auto;">
			</div>
		</div>
		<div class="row">
		...
		<div class="row">
			<div class="col-md-12">
				<div class="btn-group">
					<div class="btn-group-item" id="Import_importBtn_groupItem">
						<aui:button cssClass="btn btn-primary" type="button" name="Import_importBtn" value="Import"/>
					</div>
					
					<div class="btn-group-item">
						<aui:button cssClass="btn btn-primary" type="button" name="Import_cancelBtn" value="Close"/>
					</div>
				</div>
			</div>
		</div>
...
<%--	A.one('#<portlet:namespace /><%=htmlElement.getName()%>_uploadBtn').on('click', function(event) {
	...
	$(document).ready(function() {
		...
		$('#<portlet:namespace/>Import_file').on('change', function(event) {
			//alert('#<portlet:namespace />Import_file_change before uploadFile');
...
<%
	if(dialogMode){
%>		
		$('#<portlet:namespace/>Import_cancelBtn').on('click', function(event) {
...
%>		
	...
	function invokeImport(){
		...
		//alert("invokeImport : "+ fileEntryId);
		if(fileEntryId==''){
			alert('Please select a valid excel(xlsx) file to import!');
			return;
		}
		AUI().use('aui-base', function(A){
			//alert("Before calling /jetform.jetformbuilder/import-excel");
			...
			Liferay.Service('/jetform.jetformbuilder/import-excel', 
				...
				function(response) {
					...
					try{
						...
						var totalCtr=response['totalCtr'];
						var html="<h3><u>Status</u></h3>";
						html+="<p>Total Imported : "+response['importCtr']+"</p>";
						html+="<p>Total Failed : "+response['failedCtr']+"</p>";
						html+="<h3><u>Detail Report</u></h3>";
						for(i=1; i<=totalCtr;i++){
							var message=response['Row'+i];
							if(message!="Success"){
								html+="<p>Row "+i+" : "+message+"</p>";
							}
						}
						$("#file-input-container").hide();
						$("#status-report").html(html);
						$("#status-report").show();
						
					}catch(e){}
					$('#<portlet:namespace/>Import_importBtn').prop("disabled", true);
					$('#<portlet:namespace/>Import_importBtn').hide();
				}
		 	);
	 	});
 	}
