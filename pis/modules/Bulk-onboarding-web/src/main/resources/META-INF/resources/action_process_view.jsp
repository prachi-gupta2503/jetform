...
<%
	...
	if(search !=null){
		...
		requestList = BulkOnboardingViewHelper.getRegistrationRequestsByOnboardingId(onboardingId);
	}else{
...
<c:set var="currentAction" value='<%=targetAction %>' />

<portlet:actionURL name="selectedProcessed" var="processAll">
	<portlet:param name="action" value="${currentAction}" />
</portlet:actionURL>
...
<aui:form name="performAction" action="${processAll}" id="multi-select-form" onSubmit="validateCheckboxIsChecked(event)">
	<div class="row bg-primary text-white m-0 p-2">
		 <div class="col-9 card-heading" >
		      <h3>Disclaimer</h3>
		 </div>
		 <div class="col-3 card-heading float-right" >
	         	<a href=${backPageURL} class="btn btn-sm btn-secondary float-right"><liferay-ui:icon image="back" message ="Back" /></a>
	     </div>
	</div>
