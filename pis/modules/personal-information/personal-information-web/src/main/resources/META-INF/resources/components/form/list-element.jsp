	...
	try{
		$('#<portlet:namespace/><%=form.getId()%><%=HtmlFormAction.ACTION_ADD%>').click(function(){
			...
			if(!<%=form.getId()%>validParentPK){
				...
				showAlert("Add - <%=AnnotationUtil.getItemLabel(form.getTitle())%>",message , false, false);
				return false;
			}
...
function showAlert(alertTitle, message, error, confirmation){
	...
	var height = 200;
	var width = 460;
	LiferayUtilOpenWindow(dialogId, title, url, true, true, height, width);
}

function LiferayUtilOpenWindow(id, title, uri, centered, modal, height, width){
	Liferay.Util.openWindow({
		dialog: {
			centered: centered,
			height: height,
...
<%--	A.one('#<portlet:namespace/><%=form.getId()%>action<%=HtmlFormAction.ACTION_ADD%>').on('click', function(event) {
	...
	Liferay.provide(
		window,
		'<portlet:namespace /><%=form.getId()%><%=form.getId()%>closeDialog',
		function(data) {
			if(data){
				console.log(data);	
			}
	...
	Liferay.provide(
		...
		function(data) {
			...
			var dialog = Liferay.Util.Window.getById('<portlet:namespace/><%=form.getId()%>alertDialog');
			dialog.destroy();
		},
		['liferay-util-window']
	);	
