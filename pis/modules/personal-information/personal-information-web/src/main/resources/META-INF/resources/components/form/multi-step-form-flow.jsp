...
<%
   ...
   System.out.println("formFlowClass"+   formFlowClass);
	HtmlFormWrapper formFlow = JetFormBuilderLocalServiceUtil.rendererForm(formFlowClass);
				...
				<div class="row">
					<div class="col-md-12 mx-0">
						...
						<%
							liIndex=0;
							int linkableCount=0;
							List<HtmlElementWrapper> flowElements=formFlow.getElements();
							HtmlElementWrapper flowElement = null;
							for (int i=0; i<flowElements.size()-1;i++) {
								
								flowElement = flowElements.get(i);
								if(flowElement.isKey()){
									continue;
								}
								String liClass = "";
								if (liIndex == tabIndex) {
								...
								if(formFlowClassPK>0){
									if(flowElement.isControlType(HtmlElement.ControlType.form) || flowElement.isControlType(HtmlElement.ControlType.list)){
										//LOGGER.info(e.getFormClass());
										HtmlFormWrapper liForm = JetFormBuilderLocalServiceUtil.rendererForm(flowElement.getFormClass());
								...
								//LOGGER.info("liIndex : "+liIndex+", linkable : "+linkable+", linkableCount: "+linkableCount);
								if(linkable || linkableCount==liIndex){
									liClass=(StringUtils.isBlank(liClass)? "active":liClass);
						%>
								<li class="flow-tab <%=liClass%>" id="li-tab<%=liIndex%>"><a href="<%=tabItemURL.toString()%>&<portlet:namespace/>formFlowClassPK=<%=formFlowClassPK%>&<portlet:namespace/>tabIndex=<%=liIndex%>"><%=flowElement.getLabel()%></a></li>
						<%
								}else{
						%>
								<li class="flow-tab <%=liClass%>" id="li-tab<%=liIndex%>"><%=flowElement.getLabel()%></li>
						<%									
								}
								liIndex++;
							}
							...
							HtmlElementWrapper lastFlowElement = flowElements.get(flowElements.size()-1);
							if(liIndex==linkableCount){
								liClass=(StringUtils.isBlank(liClass)? "active":liClass);
						%>
							<li class="flow-tab <%=liClass%>" id="li-tab<%=liIndex%>"><a href="<%=tabItemURL.toString()%>&<portlet:namespace/>formFlowClassPK=<%=formFlowClassPK%>&<portlet:namespace/>tabIndex=<%=liIndex%>"><%=lastFlowElement.getLabel()%></a></li>
						<%
							}else{
						%>
								<li class="flow-tab <%=liClass%>" id="li-tab<%=liIndex%>"><%=lastFlowElement.getLabel()%></li>
						<%									
							}
						%>
...
%>						
							<liferay-util:param name="formClass" value="<%=element.getFormClass()%>" />
<%
						}
%>							
						</liferay-util:include>
