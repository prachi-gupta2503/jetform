...
String filterActions=request.getParameter("filterActions");
			...
			function(response) {
			    ...
			    $(response).each(function(idx1, object){
				    ...
				    /* Start */	
				    	...
				    	if(object[column["name"]]=='Initiate Dissolve'){
				    		initiateStatus="dissolveapprover,edit";
				    		countForfilterActions=1;
				    	}else if(object[column["name"]]=='Initiate Merge'){
				    		initiateStatus="mergeapprover,history,edit";
				    		countForfilterActions=1;
				    	  }
				    	  else if(object[column["name"]]=='Initiate Split'){
				    		initiateStatus="splitapprover,history,edit";
				    		countForfilterActions=1;
				    	  }else if(object[column["name"]]=='' && column["name"]=='orgStatus'){
				    	  initiateStatus="dissolve,split,merge,edit,remover";
				    		countForfilterActions=1;
				    	  }
				    	  else if(object[column["name"]]=='Dissolved'){
				    		initiateStatus="view";
				    		countForfilterActions=1;
				    	}else if(object[column["name"]]=='Merged'){
				    		initiateStatus="history,view";
				    		countForfilterActions=1;
				    	  }
				    	  else if(object[column["name"]]=='Splited'){
				    		initiateStatus="history,view";
				    		countForfilterActions=1;
				    	  }
				    	}
...
Liferay.provide(window, 'getActionString', function(actions, idvalue){
	...
	actionStr+="<div class=dropdown show><a class='btn btn-secondary dropdown-toggle' href='#' role='button' id='dropdownMenuLink' data-toggle='dropdown' aria-haspopup='true' aria-expanded='false'>Action</a>";
	actionStr+="<div class='dropdown-menu' aria-labelledby='dropdownMenuLink'>";
	$(actionArr).each(function(index, item){
		if(item!=''){
		
			var label=<%=form.getId()%>actionLabels[item+"_label"];
			actionStr+="<a data-key='"+idvalue+"' name='<portlet:namespace/><%=form.getId()%>"+item+"' class='row-action dropdown-item row-action-"+item+" active mr-2'  style='color:black;background-color: #f8f9fa'>"+label+"</a>";
