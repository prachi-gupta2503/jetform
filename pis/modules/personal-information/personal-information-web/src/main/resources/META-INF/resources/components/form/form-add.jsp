...
<%
	...
	String enableRedirectPage = ParamUtil.getString(request, "enableRedirectPage");
	...
	request.setAttribute("formClassPK", formClassPK);
	request.setAttribute("formId", form.getId());
	request.setAttribute(form.getId(), form);
	for(HtmlElementWrapper element: form.getAllElements()){
	...
	if(childFormCount>0){
		request.setAttribute("parentFormId", form.getId());
	}
	//pageContext.setAttribute("htmlForm", form);
	//request.setAttribute("htmlForm", form);
	
/* updated by prachi  on 02-09-2022 */

	ExpandoBridge expandoBridge=null;
	Class clazz=FormEntityCache.getFormClass(form.getEntityClass());
	try{
		if(StringUtils.isNotBlank(formClassPK)){
			Method expMethod=clazz.getMethod("getExpandoBridge", new Class[]{});
	...
	LOGGER.info("expandoBridge::  "+expandoBridge);
	request.setAttribute(form.getId()+"expandoBridge", expandoBridge);
/*  */	
%>
...
<%	LOGGER.info("redirectPageTwo : "+redirectPage); %>
