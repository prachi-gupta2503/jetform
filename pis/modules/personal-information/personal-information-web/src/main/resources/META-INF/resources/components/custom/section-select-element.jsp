...
<%
	if(viewOnlyMode){
%>
		<aui:select type="<%=htmlElement.getControlType()%>" name="<%=form.getElementNamespace()+htmlElement.getName()%>" label="" multiple="<%=htmlElement.isControlType(HtmlElement.ControlType.multiSelect)%>" disabled="<%=disabled %>">
		</aui:select>
<%
	} else{
%>
