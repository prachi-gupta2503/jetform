...
<%
if(htmlElement.isControlType(HtmlElement.ControlType.email) || validType.equals(HtmlElement.ValidType.email.name())){
%>	
...
if(htmlElement.isControlType(HtmlElement.ControlType.phone) || validType.equals(HtmlElement.ValidType.phone.name())){
%>	
...
<aui:validator errorMessage="Invalid field value" name="custom">
   function(val, fieldNode, ruleValue) {
   		console.log("custom validation is called");
   		if(val=="" || val==<portlet:namespace/><%=childFormNamespace+htmlElement.getName()%>ValidatedValue){
			return true;
		}
		var errorMessage="";
...
%>
		...
		//checkUnique<portlet:namespace/><%=childFormNamespace+htmlElement.getName()%>(val);
	    console.log("before return :"+new Date()+", "+ <portlet:namespace/><%=childFormNamespace+htmlElement.getName()%>Unique);
	    if(<portlet:namespace/><%=childFormNamespace+htmlElement.getName()%>Unique){
	    	<portlet:namespace/><%=childFormNamespace+htmlElement.getName()%>ValidatedValue=val;
	    }
	    uniqueValue = <portlet:namespace/><%=childFormNamespace+htmlElement.getName()%>Unique;
<%
}
%>	    
	    //alert(uniqueValue);
...
%>         	
        ...
        if(!uniqueValue || !validValue){
	        var myFormValidator = Liferay.Form.get('<portlet:namespace /><%=parentFormId%>').formValidator;
			...
			_ruleData.<%=childFormNamespace+htmlElement.getName()%>_custom = errorMessage;	
