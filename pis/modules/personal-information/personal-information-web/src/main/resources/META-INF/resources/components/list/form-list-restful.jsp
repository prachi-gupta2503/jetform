...
boolean hideActions=ParamUtil.getBoolean(request, "hideActions", false);
boolean hideRowActions=ParamUtil.getBoolean(request, "hideRowActions", false);

//System.out.println("hideActions............................."+hideActions);
try{
...
//boolean profileEditor = ParamUtil.getBoolean(request, "profileEditor",true);
//System.out.println("profileEditor:: "+ profileEditor);
...
while(paramNames.hasMoreElements()){
	...
	if(paramName.startsWith("formAction")){
		if(paramValue!=null ){
			...
			String actionName=paramName.substring("formAction[".length(), paramName.length()-1);
			actionsParamMap.put(actionName.toLowerCase(), paramValue);
			System.out.println(actionsParamMap);
...
<%
	...
	for(HtmlElementWrapper element:elements){
		...
		//LOGGER.info("element.getName() : "+element.getName()+ ", element.getControlType() : "+element.getControlType()+ ", "+element.isControlType(HtmlElement.ControlType.fileUpload));
	...
	<div id="<portlet:namespace/><%=form.getId()%>dataTableContainer" class="col-md-12">
		<div class="card">
			<div class="card-body" style="overflow:auto; padding: 0 0 0 0 !important;">
				...
				<table id="<portlet:namespace/><%=form.getId()%>dataTable" class="stripe hover mb-5 pt-2" style="width:96%">
				</table>
			...
			'/jetform.jetformbuilder/read-objects-as-json', 
 		{
			...
			function(response) {
			    ...
			    $(response).each(function(idx1, object){
				    ...
				    /* updated by Abhishek Sharma on 14-10-2022	 */	
				    /* Start */	
				    	if(countForHideActions>0){
					    	dataRow[idx2+1]="";
					    	countForHideActions=0;
				    	 }else{
				    		dataRow[idx2+1]=object[column["name"]];
				    	 }
				    	if(<%=hideActions %> && object[column["name"]]=='rejected' ){
				    	countForHideActions=1;
				    	  }
				    	 /* End */
				    	  if(countForfilterActions>0){
					    	dataRow[idx2+1]=initiateStatus;
					    	countForfilterActions=0;
				    	 }
				    	
				    	  if(object[column["name"]]=='Yes' && column["name"]=='deleted'){
				    	   initiateStatus="";
				    		countForfilterActions=1;
				    	  }		
				    	}
			    	});
			    	
			    	data[idx1]=dataRow;
			    	//console.log("data[idx1]:  "+data[idx1]);
			    });
...
<%
			if(exportExcel){ 
%>	
	               dom: 'B<"dataTables_length_filter"lf><"#<portlet:namespace/><%=form.getId()%>dataTable_table.data-table-parent"t><"dataTables_info_paginate"ip>',
	                buttons: [{
                         extend: 'excel',
                        text: 'Export ',
...
%>              
				"dom": '<"dataTables_length_filter"lf><"#<portlet:namespace/><%=form.getId()%>dataTable_table.data-table-parent"t><"dataTables_info_paginate"ip>',
<%
			}
%>	                
                    data: data,
...
<%
	...
	if(rowActions.size()>0 && ! hideRowActions) {
...
Liferay.provide(window, 'getActionString', function(actions, idvalue){
	...
	<%--var actionStr+="<a data-key=\"'+$('<div/>').text(data).html()+'\" name=\""+actionItemName+"\" class=\""+activeActionClass+"\">"+action.getLabel()+"</a>";
	 --%>
	 //console.log("actions :"+actions);5
	 //console.log("idvalue :"+idvalue);
	var actionArr= actions.split(",");
	$(actionArr).each(function(index, item){
		var actionValue= item.split(":");
		//console.log("item :"+item);
		let actionValueArray = [];
		$(actionValue).each(function(index, val){
		  actionValueArray.push(val);
		
		});
		var label=<%=form.getId()%>actionLabels[actionValueArray[0]+"_label"];
			//console.log("label :"+label);
		if(actionValueArray[0]!=''){
			if(actionValueArray[1]!=='false'){
			   if(actionValueArray[2]!=='false'){
				
				  		actionStr+="<a data-key='"+idvalue+"' name='<portlet:namespace/><%=form.getId()%>"+actionValueArray[0]+"' class='row-action row-action-"+actionValueArray[0]+" active mr-2'>"+label+"</a>";
					<%-- actionStr+=`<a data-key='${idvalue}' 
					               name='<portlet:namespace/><%=form.getId()%>${actionValueArray[0]}' 
					               class='row-action row-action-${actionValueArray[0]} active mr-2'>${label}</a>`;
					 --%>
					//console.log("actionStr :"+actionStr);
			   }else{
				  actionStr+="<a data-key='"+idvalue+"' name='<portlet:namespace/><%=form.getId()%>"+actionValueArray[0]+"inactive' class='row-action row-action-"+actionValueArray[0]+" inactive mr-2'>"+label+"</a>"
...
<%
	if(MapUtils.isNotEmpty(searchFilters)){
		for(Map.Entry<String, String> entry:searchFilters.entrySet()){
%>
		url+="&<portlet:namespace/><%=entry.getKey() %>.hidden =<%=entry.getValue()%>"
<%
		}
	}
...
Liferay.provide(window,'<%=form.getId()%>ShowSuccessAlert',function(message){
	...
	$('#<%=form.getId()%>alertSuccess').delay( 3000 ).fadeOut( 400 );
...
Liferay.provide(window,'<%=form.getId()%>ShowErrorAlert', function(message){
	...
	$('#<%=form.getId()%>alertError').delay( 3000 ).fadeOut( 400 );
});
...
Liferay.provide(window, '<portlet:namespace/>propagateSearch',function(searchParam){
