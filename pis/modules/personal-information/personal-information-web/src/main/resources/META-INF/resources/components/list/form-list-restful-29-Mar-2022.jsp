...
<%
	...
	int ix=1;
	for(HtmlElementWrapper hew: form.getElements()){
		if(!hew.isKey() && !hew.isHidden() && hew.isLink()){
%>
			{
				 'targets': <%=ix%>,
				 'render': function (data, type, full, meta){
			          return '<a href="<%=hew.getLinkAction()%>" name="<%=hew.getName()%>[]" value="' + $('<div/>').text(data).html() + '" onclick="<portlet:namespace/><%=form.getId()%><%=hew.getLinkAction()%>">';
			      }
			},
<%
		}
...
function <%=form.getId()%>initLinkCols(){
<%
	int ctr=0;
	for(HtmlElementWrapper e: form.getElements()){
		if(e.isLink()){
%>
		<%=form.getId()%>linkCols[i]={"index":<%=ctr%>, "name":"<%=e.getName()%>", "linkAction":"<%=e.getLinkAction()%>"};
<%		
		}
	}
%>	
...
<!--
function propagateSearch(searchParam){
