...
*/
	if(htmlElement.isControlType(HtmlElement.ControlType.hidden) || htmlElement.isKey() || isPresetHidden){
		...
		if((!StringUtils.equalsIgnoreCase(presetHiddenField, "true") || !StringUtils.equalsIgnoreCase(presetHiddenField, "yes")) && StringUtils.isEmpty(savedValue)){
			savedValue=presetHiddenField;
...
%>
		<aui:input type="hidden" name="<%=childFormNamespace+htmlElement.getName()%>" value="<%=savedValue%>" cssClass="<%=keyCssClass%>"/>
<%
	}
}
...
<%--	<aui:input type="hidden" name="<%=form.getElementNamespace()+form.getKeyElement().getName()%>" value="<%=form.getPrimaryKey()%>"/> --%>
	<aui:input type="hidden" name="<%=childFormNamespace+"formClassPK"%>" value="<%=form.getPrimaryKey()%>"/>		
<%
}
%>	

<aui:input type="hidden" name="<%=childFormNamespace+"formClass"%>" value="<%=form.getFormClass()%>"/>
