	...
	for(HtmlElementWrapper element:elements){
		...
		if(element.isControlType(HtmlElement.ControlType.form)){
			if(StringUtils.isNotBlank(element.getListField())){
				...
				HtmlFormWrapper childForm = JetFormBuilderLocalServiceUtil.getHtmlFormByClass(element.getFormClass());
	...
	if(rowActions.size()>0){
		...
		if(profileEditor){
			for(HtmlFormActionWrapper action:rowActions){
				...
				actionStr+="<a data-key=\"'+data+'\" name=\""+actionItemName+"\" class=\"row-action row-action-"+action.getName()+"\" title=\""+action.getTitle()+"\">"+action.getLabel()+"</a>";
