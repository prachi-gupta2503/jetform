...
import com.adjecti.pis.liferay.model.impl.EmployeeDesignationImpl;
import com.adjecti.pis.liferay.model.impl.EmployeeDesignationModelImpl;
 ...
 * @generated
 */
@Component(service = EmployeeDesignationPersistence.class)
public class EmployeeDesignationPersistenceImpl
	extends BasePersistenceImpl<EmployeeDesignation>
	implements EmployeeDesignationPersistence {

	/*
	 * NOTE FOR DEVELOPERS:
	 *
	 * Never modify or reference this class directly. Always use <code>EmployeeDesignationUtil</code> to access the employee designation persistence. Modify <code>service.xml</code> and rerun ServiceBuilder to regenerate this class.
	 */
	public static final String FINDER_CLASS_NAME_ENTITY =
		EmployeeDesignationImpl.class.getName();
	 ...
	 */
	@Override
	public List<EmployeeDesignation> findByEmployeeId(long employeeId) {
	 ...
	 */
	@Override
	public List<EmployeeDesignation> findByEmployeeId(
	...
	public EmployeeDesignation findByEmployeeId_First(
		...
		EmployeeDesignation employeeDesignation = fetchByEmployeeId_First(
	...
	public EmployeeDesignation fetchByEmployeeId_First(
		...
		List<EmployeeDesignation> list = findByEmployeeId(
	...
	public EmployeeDesignation findByEmployeeId_Last(
		...
		EmployeeDesignation employeeDesignation = fetchByEmployeeId_Last(
			employeeId, orderByComparator);
	...
	protected EmployeeDesignation getByEmployeeId_PrevAndNext(
	...
	public EmployeeDesignationPersistenceImpl() {
		...
		setModelImplClass(EmployeeDesignationImpl.class);
	...
	public void clearCache() {
		entityCache.clearCache(EmployeeDesignationImpl.class);
	...
	public void clearCache(EmployeeDesignation employeeDesignation) {
		entityCache.removeResult(
			EmployeeDesignationImpl.class, employeeDesignation);
	}
	...
	public void clearCache(Set<Serializable> primaryKeys) {
		...
		for (Serializable primaryKey : primaryKeys) {
			entityCache.removeResult(EmployeeDesignationImpl.class, primaryKey);
		}
	}
	...
	public EmployeeDesignation create(long employeeDesignationId) {
		...
		employeeDesignation.setCompanyId(CompanyThreadLocal.getCompanyId());
	...
	public EmployeeDesignation remove(Serializable primaryKey)
		...
		try {
			...
			EmployeeDesignation employeeDesignation =
				(EmployeeDesignation)session.get(
			...
			if (employeeDesignation == null) {
				...
				throw new NoSuchEmployeeDesignationException(
					_NO_SUCH_ENTITY_WITH_PRIMARY_KEY + primaryKey);
			}

			return remove(employeeDesignation);
		}
		catch (NoSuchEmployeeDesignationException noSuchEntityException) {
	...
	protected EmployeeDesignation removeImpl(
		...
		try {
			...
			if (!session.contains(employeeDesignation)) {
				employeeDesignation = (EmployeeDesignation)session.get(
					EmployeeDesignationImpl.class,
	...
	public EmployeeDesignation updateImpl(
		...
		if (!(employeeDesignation instanceof EmployeeDesignationModelImpl)) {
			InvocationHandler invocationHandler = null;

			if (ProxyUtil.isProxyClass(employeeDesignation.getClass())) {
				invocationHandler = ProxyUtil.getInvocationHandler(
					employeeDesignation);

				throw new IllegalArgumentException(
					"Implement ModelWrapper in employeeDesignation proxy " +
						invocationHandler.getClass());
			}

			throw new IllegalArgumentException(
				"Implement ModelWrapper in custom EmployeeDesignation implementation " +
					employeeDesignation.getClass());
		}

		EmployeeDesignationModelImpl employeeDesignationModelImpl =
			(EmployeeDesignationModelImpl)employeeDesignation;
		...
		if (isNew && (employeeDesignation.getCreateDate() == null)) {
			if (serviceContext == null) {
				employeeDesignation.setCreateDate(now);
			}
			else {
		...
		if (!employeeDesignationModelImpl.hasSetModifiedDate()) {
			if (serviceContext == null) {
				employeeDesignation.setModifiedDate(now);
			}
			else {
		...
		try {
			...
			if (isNew) {
				...
				employeeDesignation = (EmployeeDesignation)session.merge(
					employeeDesignation);
			}
		}
		catch (Exception exception) {
		...
		entityCache.putResult(
			EmployeeDesignationImpl.class, employeeDesignationModelImpl, false,
			true);
	...
	/**
	 * Returns the employee designation with the primary key or throws a <code>com.liferay.portal.kernel.exception.NoSuchModelException</code> if it could not be found.
	 *
	 * @param primaryKey the primary key of the employee designation
	...
	public List<EmployeeDesignation> findAll(
		...
		if (list == null) {
			...
			if (orderByComparator != null) {
				...
				sb.append(_SQL_SELECT_EMPLOYEEDESIGNATION);
		...
		if (count == null) {
			...
			try {
				...
				Query query = session.createQuery(
					_SQL_COUNT_EMPLOYEEDESIGNATION);
	...
	protected String getSelectSQL() {
		return _SQL_SELECT_EMPLOYEEDESIGNATION;
	}

	@Override
	protected Map<String, Integer> getTableColumnsMap() {
		return EmployeeDesignationModelImpl.TABLE_COLUMNS_MAP;
	}
	...
	public void activate(BundleContext bundleContext) {
		...
		_argumentsResolverServiceRegistration = _bundleContext.registerService(
			...
			new EmployeeDesignationModelArgumentsResolver(),
			MapUtil.singletonDictionary(
				"model.class.name", EmployeeDesignation.class.getName()));
	...
	public void deactivate() {
		entityCache.removeCache(EmployeeDesignationImpl.class.getName());
	...
	private static final String _SQL_SELECT_EMPLOYEEDESIGNATION =
		"SELECT employeeDesignation FROM EmployeeDesignation employeeDesignation";

	private static final String _SQL_SELECT_EMPLOYEEDESIGNATION_WHERE =
		"SELECT employeeDesignation FROM EmployeeDesignation employeeDesignation WHERE ";

	private static final String _SQL_COUNT_EMPLOYEEDESIGNATION =
		"SELECT COUNT(employeeDesignation) FROM EmployeeDesignation employeeDesignation";

	private static final String _SQL_COUNT_EMPLOYEEDESIGNATION_WHERE =
		"SELECT COUNT(employeeDesignation) FROM EmployeeDesignation employeeDesignation WHERE ";

	private static final String _ORDER_BY_ENTITY_ALIAS = "employeeDesignation.";

	private static final String _NO_SUCH_ENTITY_WITH_PRIMARY_KEY =
		"No EmployeeDesignation exists with the primary key ";

	private static final String _NO_SUCH_ENTITY_WITH_KEY =
		"No EmployeeDesignation exists with the key {";

	private static final Log _log = LogFactoryUtil.getLog(
		EmployeeDesignationPersistenceImpl.class);
	...
	private static class EmployeeDesignationModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			EmployeeDesignationModelImpl employeeDesignationModelImpl =
				(EmployeeDesignationModelImpl)baseModel;

			long columnBitmask =
				employeeDesignationModelImpl.getColumnBitmask();

			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(
					employeeDesignationModelImpl, columnNames, original);
			}
		...
		private Object[] _getValue(
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] =
						employeeDesignationModelImpl.getColumnOriginalValue(
							columnName);
				}
				else {
					arguments[i] = employeeDesignationModelImpl.getColumnValue(
						columnName);
				}
			}
