...
%>
	...
	console.log("<%=form.getId()%>initFilters()");
...
<%--	if(<%=form.getId()%>dataTableFilterSearchField!='' && <%=form.getId()%>dataTableFilterSearchValue!=''){
		<%=form.getId()%>updateTableWithSearchFilter(<%=form.getId()%>dataTableFilterSearchField, <%=form.getId()%>dataTableFilterSearchValue);
		return;
	}
 --%>	
	AUI().use('aui-base', function(A){
...
function <%=form.getId()%>initHeaders(){
	...
	$(<%=form.getId()%>columns).each(function(index, column){
		if(column.key){
			<%=form.getId()%>headers[index+1]={"title": "<input type='checkbox' disabled>"};
		}else{
...
function <%=form.getId()%>updateTableWithSearchFilter(searchField, searchValue){
	alert("<%=form.getId()%>updateTableWithSearchFilter called with searchField : "+searchField+", value : "+searchValue);
	<%=form.getId()%>dataTableFilterSearchField=searchField;
	<%=form.getId()%>dataTableFilterSearchValue=searchValue;
	...
	AUI().use('aui-base', function(A){
		Liferay.Service(
			...
			function(response) {
				console.log("updateTableWithSearchFilter response : ");
				console.log(response);
			    var data=[];
