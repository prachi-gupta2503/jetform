...
%>		
	<i class="fa fa-exclamation-triangle" style="color:red; font-size: 1.5rem;"></i>
			Are you sure you want to active the <%=AnnotationUtil.getItemLabel(form.getTitle())%>?
<%
	}else{
%>		
	<i class="fa fa-exclamation-triangle" style="color:black; font-size: 1.5rem;"></i>
			No valid <%=AnnotationUtil.getItemLabel(form.getTitle())%> record to active!
<%
	}
%>	
		</div>
...
function deleteEntity(entityClass, entityClassPK){
	Liferay.Service('/jetform.jetformbuilder/un-delete-entity',
		{
		...
		function(response) {	
			...
			if(status){
				message="Record actived successfully.";
			}else{
				
				message="Record activation failed.";
				error=true;
				refresh=false;
			}
