...
HtmlFormActionWrapper approveAction=new HtmlFormActionWrapper();
approveAction.setName("approve");
approveAction.setLabel("<i class=\"fa fa-check\" aria-hidden=\"true\"></i>");

int approvePos=0;

for(HtmlFormActionWrapper aw:rowActions){
	if(aw.getName().equals(HtmlFormAction.ACTION_DELETE)){
		break;
	}
	approvePos++;
}
if(rowActions.size()==approvePos){
	rowActions.add(approveAction);
}else{
	rowActions.add(approvePos, approveAction);
}
