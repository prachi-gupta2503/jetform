...
%>
<b>Error:</b>
Form not initialized due to formClass is "<%=formClass%>" and Form definition is "<%=form == null ? "null" : form.getTitle()%>".
<%
	} else {
	...
	<div class="card-header">
		<div class="row">
			...
			<div class="col-md-4">
				...
				<a style="float: right;" class="text-white fs-5" href="<%=addActionObj.getAction()%>" title="<%=StringUtils.capitalise(addActionObj.getName())%>"
					id="<portlet:namespace/><%=form.getId()%>action<%=HtmlFormAction.ACTION_ADD%>"><i class="fa fa-plus mr-2" aria-hidden="true"></i>Add <%=AnnotationUtil.getItemLabel(form.getTitle())%></a>
						...
						<tr>
							...
							<%
								for (HtmlElementWrapper ele : elements) {
									if (!ele.isKey() && !ele.isControlType(HtmlElement.ControlType.hidden)
											&& !ele.isControlType(HtmlElement.ControlType.password) && ele.isListable()) {
							%>
									...
									for (HtmlElementWrapper ele : elements) {
										if (!ele.isKey() && !ele.isControlType(HtmlElement.ControlType.hidden)
											...
											if (relation != null && !relation.isNon()) {
												...
												if (entityId != null && !entityId.isEmpty() && !"0".equals(entityId)
														&& relation.getModelClass() != null) {
													KeyValue kv = null;
													
													//if(String.valueOf(relation.getModelClass())!=null){
													//	FormEntityCache.registerFormClass(relation.getModelClass());
													kv = jetFormBuilderLocalService.getRelationObjectAsKeyValue(
															relation.getModelClass(), entityId, relation.getValueField(),
			...
			"drawCallback": function (oSettings) {
		        $(".dataTables_filter").each(function () {
		        	...
		        	if(!dataTableFilterUpdated){
		        		$(this).append("<i class='fa fa-search' style='color: white; margin-left: -20px;' aria-hidden='true'></i>");
<%
	if (form.isAdvanceFilter()) {
%>		        		
