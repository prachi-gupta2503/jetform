...
long requestedDate = ParamUtil.getLong(request, "date", System.currentTimeMillis());
...
long date = requestedDate;
int year = cal.get(java.util.Calendar.YEAR);
int month = cal.get(java.util.Calendar.MONTH);


if(calendarDate != null && !calendarDate.isEmpty() && !calendarDate.trim().isEmpty()) {
	String arr[] = calendarDate.split("-");
	year = Integer.parseInt(arr[0]);
	/*
	Minus 1 because liferay calendar index start from 0 
	*/
	month = Integer.parseInt(arr[1])-1;
    cal.set(java.util.Calendar.YEAR, year);
    cal.set(java.util.Calendar.MONTH, month);
    cal.set(java.util.Calendar.DAY_OF_MONTH, 01);
	date = cal.getTime().getTime();
} 
/*
Get string year-month to display in date-picker
add 1 because previously subtracted 
*/
month = month+1;
...
<%--CARD START --%>
<div class="card  news-box mb-0 calendar-outer-card-height">
	<div class="card-header title-card-header">
		<i class="fas fa-calendar-week mr-2"></i> Calendar
	</div>
	<div class="card-body py-2 calendar-inner-card-height">
<%--CARD START --%>	
...
<%-- Code For Date Picker Starts --%>
			...
			<portlet:renderURL var="customDateRenderURL">
			    <portlet:param name="mvcPath" value="/view_calendar.jsp"></portlet:param>
			</portlet:renderURL>
			
			
			<aui:form method="post" action="<%=customDateRenderURL%>" id="calendarDateForm">
				<div class="row ">
					<%--<div class="<%= !displaySchedulerOnly?"offset-md-7":"col-md-7" %> "><%= !displaySchedulerOnly?"":"Calendar"%></div> --%>
					<div class="col-md-12 ">
						<%--<div class="input-group mb-3"> --%>
						<div class="input-group mb-1">
							<input type="month" class='form-control rounded-0 calendarDate'	id="customDate" name="<portlet:namespace />calendarDate"	value="<%=calendarDate%>">
							<input type="hidden" id="activeView" name="<portlet:namespace />activeView" value="<%=activeView%>">
							<!-- <div class="input-group-append">
								<button class="btn btn-primary rounded-0" type="submit"	id="button-addon2">Submit</button>
							</div> -->
						</div>
					</div>
				</div>
...
<%-- Scripts For Date Picker Starts --%>